...
?>
 <?php
   ...
   $erreur="";
   if(isset($valider)){
      if(empty($cin)) $erreur="CIN laissé vide!";
      ...
      elseif(empty($adresse)) $erreur="adresse laissée vide!";
      
      else{
         include("connexion.php");
         $sel=$pdo->prepare("select id from etudiant where cin=? limit 1");
         $sel->execute(array($login));
         $tab=$sel->fetchAll();
         if(count($tab)>0)
            $erreur="cin existe déjà!";
         else{
            $ins=$pdo->prepare("insert into etudiant(cin,nom,prenom,pass,repass,Classe,adresse) values(?,?,?,?,?,?,?)");
            if($ins->execute(array($cin,$nom,$prenom,$pass,$repass,$Classe,$adresse)));
               header("location:login.php");
         }   
      }
   }
...
<html lang="en">
  ...
  <body>
    ...
    <?php include('ajouterEtudiant.php') ; ?>
  </body>
</html>
